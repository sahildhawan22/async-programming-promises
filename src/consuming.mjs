...
export function get(){
 /*  axios.get("http://localhost:3000/orders/1").then(
  ...
  //then accepts only 1 function as its parameter and is used to handle the FULFILLED state of the promise.
  axios.get("http://localhost:3000/orders/1").then(
    function({data}){
...
export function getCatch(){
  ...
  */
  ...
  /* 
    The above results in the following 2 errors:
    ...
    While the first error is expected, why are we facing the second error? It's because we're not handling the REJECTED state of promise. To handle the REJECTED state, chain the catch function like below:
  */
...
export function chain(){
  ...
  */
  ...
  /* 
    This time instead of seeing the order on the screen by runnin g the same code snippet as in get() function, we'll print which city is order being shipped to. For that, we first need to load the order and then see its shipping address in the next API call.
    See doc file.
  */
...
export function chainCatch(){
  /* 
    The following code catches error in the catch chained snippet. Since it's the last snippet in the stack, it can catch error of both the then before it. Try skipping return in the first then or making data.my.city in the next then.
  */
